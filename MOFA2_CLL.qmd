...
Make sure that you have installed the [MOFA2](https://github.com/bioFAM/MOFA2#installation) and the [MOFAdata](https://www.bioconductor.org/packages/release/data/experiment/html/MOFAdata.html) package. This should be the case if you are using the VM we provided.   
...
reticulate::py_require("mofapy2", python_version = "3.12.12")
reticulate::py_config()
# run_mofa(..., use_basilisk=FALSE)
```
...
CLL_metadata = fread("./metadata/sample_metadata.txt")
...
model_opts
```
Please use `?get_default_model_options()` to check more detailed explanation of each option. 
...
train_opts
```
We want MOFA to automatically select the number of meaningful factors by giving a threshold of the minimal variance a factor should explain for at lease one view. In order to do this, we changed the `drop_factor_threshold` to 0.02 (2% variance explained).  

We can also change the `convergence_mode` to "slow" to have better convergence.  

Please use `?get_default_training_options()` to check more detailed explanation of each option.  
...
drug_metadata = fread("./metadata/drugs.txt.gz")
...
gene_metadata = fread("./metadata/Hsapiens_genes_BioMart.87.txt.gz")
gene_metadata[,symbol:=ifelse(symbol=="",ens_id,symbol)]
tmp = gene_metadata$symbol; names(tmp) = gene_metadata$ens_id
updated_features_names[["mRNA"]] = tmp[features_names(MOFAobject)[["mRNA"]]]
...
- **Factor 1 and 3** captures a source of variability that is present in more than 3 data modalities. Thus, they may represent etiologies that is important for the disease.  
...
```
Several factors show associations with survival outcome (whether the patients were deceased). We will explore association with clinical measurements later in the tutorial.  
...
It seems a few factors (e.g. Factor 2, 4, 5 and 6) have clear gene set signatures.
```{r}
...
By inspecting the variance explained plot, we can see that the RNA expression is capturing most of the sources of variation in this data set. The *ex vivo* drug response assay also captures a lot of variability, but it is much harder to obtain from patient cohorts than RNA expression data. The other two data modalities are less informative: DNA methylation data is only active in Factor 1, 7 and 9 and Somatic mutations are only associated with Factors 1 and 3.  
...
In this case we have three mutations that have large weight. One of them is the trisomy of chromosome 12, which is the [second most important clinical marker in CLL](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6058775/)!  
...
### (Q) Characterise Factor 2 based on the GSEA results?

Plotting the GSEA results for Factor 2 reveals that this Factor is capturing differences in the stress response of the blood cells. We have significantly enriched pathways such as cellular response to stress or heat shock response.
```{r}
plot_enrichment(enrichment.results, factor = 2, max.pathways = 15)
```

The top genes that are driving this pathway are Heat Shock Proteins and some inflammatory markers such as IFNG and TNF.
```{r}
plot_top_weights(MOFAobject, 
  view = "mRNA", 
  factor = 2, 
...
It looks like these genes have a positive weight, which means that they have higher levels in the samples with positive Factor 2 values:
```{r}
plot_data_scatter(MOFAobject, 
  factor = 2, 
  view = "mRNA",
...
Out of the 5 top drugs, the target category for 5 of them is Reactive Oxygen Species, which are closely related to stress response mechanisms.
```{r}
...
Factor 1 (as expected), Factor 3, 7 and 8 are all statistically associated (P<0.01) with time to next treatment.
